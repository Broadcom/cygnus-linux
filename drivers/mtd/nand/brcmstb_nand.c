...
/* Per chip-select offset for v4.0 on CS0 only */
				     ...
				     enum brcmnand_cs_reg reg)
{
	...
	/* NAND v4.0 controller has a different CS0 offset layout */
	else if (ctrl->nand_version == 0x0400 && cs == 0)
...
{
	brcmnand_rmw_reg(ctrl, BRCMNAND_CS_SELECT, BIT(29), 29, 0);
}
...
static int brcmnand_waitfunc(struct mtd_info *mtd, struct nand_chip *this)
{
	...
	ctrl->cmd_pending = 0;
	return brcmnand_read_reg(ctrl, BRCMNAND_INTFC_STATUS) & 0xff;
}
				 ...
				 bool last_op)
{
	...
	dev_dbg(&host->pdev->dev, "ll_op cmd %#x\n", tmp);
...
static int brcmnand_verify_erased_page(struct mtd_info *mtd,
				       struct nand_chip *chip, void *buf,
				       u64 addr)
{
			 ...
			 u64 addr, unsigned int trans, u32 *buf, u8 *oob)
{
	...
	if (mtd_is_eccerr(err)) {
		int ret = brcmnand_verify_erased_page(mtd, chip, buf, addr);
			  ...
			  u64 addr, const u32 *buf, u8 *oob)
{
	...
	for (i = 0; i < trans; i++, addr += FC_BYTES) {
		...
		if (buf)
			for (j = 0; j < FC_WORDS; j++, buf++)
				brcmnand_write_fc(ctrl, j, cpu_to_le32(*buf));
		else if (oob)
...
static int brcmnand_setup_dev(struct brcmnand_host *host)
{
	...
	case 1024:
		...
		break;
	default:
		dev_err(&host->pdev->dev, "unsupported ECC size: %d\n",
			chip->ecc.size);
		return -EINVAL;
	}
